/*******************************************************************************
 Script:       uc_calibration_catch_per_trip_part2.do
 Purpose:      Assembles the per-iteration calibration catch-draw files that the
               ...
               (by mode and month), and saves one calib_catch_draws_uc_`i'.dta.
 Inputs:       $misc_data_cd/fes_person_final_2023`w'.dta (waves 1-6),
               $misc_data_cd/directed_trip_draws.csv,
               $misc_data_cd/trip_costs.dta,
               $calib_catch_draws_cd/calib_catch_draws_raw_uc_`i'.dta (copula output).
 Outputs:      $misc_data_cd/angler_dems.dta,
               $calib_catch_draws_cd/calib_catch_draws_uc_`i'.dta (i = 1..$ndraws).
 Dependencies: Globals $misc_data_cd, $calib_catch_draws_cd, $ndraws (set in
               ...
               copula_no_mrip_uncertainty.R, which writes calib_catch_draws_raw_uc_*.
...
quietly forvalues i=1/$ndraws {
	 noisily disp "Draw `i' started"
        ...
        *--------------------------------
        ...
        preserve
            u "$calib_catch_draws_cd\calib_catch_draws_raw_uc_`i'.dta", clear
        ...
        * Merge sampled catch onto trips by (g,gid)
		...
		save "$calib_catch_draws_cd\calib_catch_draws_uc_`i'.dta", replace
