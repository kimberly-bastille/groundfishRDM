...
ProjectedNAASaveFile<-glue("TEST_GOM_Haddock_projected_NAA_{data_version}")
HistoricalNAASaveFile<-glue("TEST_GOM_Haddock_historical_NAA_{data_version}")
...
SSB_logmean<-std1[[1]]$log_SSB

SSB_logsd<-std1[[2]]$log_SSB

# exponentiate
SSB<-exp(SSB_logmean)
SSB2026Assess<-SSB[RowPick]
...
message("E[ln(NAA)], taken over ", num_NAA_draws, "draws:")

log_NAA2026<-log(NAA2026)
round(colMeans(log_NAA2026),3)

message("ln(E[(NAA)]), taken over ", num_NAA_draws, "draws:")

mean_NAA2026<-NAA2026 %>%
  colMeans(NAA2026)
round(log(mean_NAA2026),3)
