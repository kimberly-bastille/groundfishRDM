################################################################################
################################################################################
# Script:       copula_no_mrip_uncertainty.R
...
#               binomial whose mean and dispersion (theta) are fixed to the
#               survey point estimates, completely omitting sampling uncertainty.
...
#'   non-positive or non-finite inputs and capping theta at theta_cap.
...
#' @description Simulates n_draws x n_sim keep/release outcomes. Survey sampling
#'   uncertainty is removed; simulated values strictly use point estimates for mean
#'   and theta.
run_species_copula_sim <- function(full_df,
  ...
  simulate_one_margin <- function(df, varname, se_varname, dom, out_prefix, fixed_zero_side = c("rel", "keep")) {
    ...
    while (i <= n_draws) {
      # Use strict point estimates without uncertainty simulation
