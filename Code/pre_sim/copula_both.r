################################################################################
################################################################################
# Script:       copula_both.R
# Purpose:      Estimates the calibration-year joint distribution of harvest
#               ("keep") and discards ("rel") per trip for cod and haddock.
#               Accepts a flag (uncertain_flag) from the command line:
#                 - 0: Includes MRIP survey sampling uncertainty (draws mu and
#                      theta via bootstrap replicate weights).
...
if (length(args) != 2) {
  stop("Error: This script requires exactly two arguments: n_draws and uncertain_flag.", call. = FALSE)
}

n_draws        <- as.numeric(args[1])
uncertain_flag <- as.numeric(args[2])

cat("Number of model iterations selected:", n_draws, "\n")
cat("Uncertainty behavior (0 = simulate uncertainty, 1 = use fixed point estimates):", uncertain_flag, "\n")
...
get_rep_var <- function(df, varname, rep_design) {
  ...
  if (is.null(rep_wgts)) return(numeric(0))
  if (is.vector(rep_wgts)) rep_wgts <- matrix(rep_wgts, ncol = 1)
  ...
  out <- sapply(seq_len(ncol(rep_wgts)), function(i) {
    ...
    svy_rep <- survey::svydesign(
      ids = ~psu_id, strata = ~strat_id, weights = ~rep_wgt_tmp,
      nest = TRUE, data = df_rep
    )
    val <- try(
      as.numeric(coef(survey::svyvar(stats::as.formula(paste0("~", varname)), svy_rep, na.rm = TRUE))),
      silent = TRUE
    )
...
safe_theta <- function(mu, var, theta_cap = 1000) {
  ...
  if (length(mu) == 0 || length(var) == 0) return(theta_cap)
...
weighted_sample_for_copula <- function(df, value_cols, n_sim, weight_col = "wp_int", jitter_ties = FALSE) {
  if (is.null(df) || nrow(df) == 0) stop("weighted_sample_for_copula: df has zero rows.")
  w <- df[[weight_col]]
  if (is.null(w)) stop("weighted_sample_for_copula: weight column not found.")
  w[is.na(w) | !is.finite(w) | w < 0] <- 0
  if (sum(w) <= 0) stop("weighted_sample_for_copula: all sampling weights are zero or missing.")

  idx <- sample.int(n = nrow(df), size = n_sim, replace = TRUE, prob = w)
...
cap_with_resample <- function(x, max_x) {
  ...
  while (excess > 0) {
    ...
    chosen_pos <- sample.int(n = length(candidates), size = 1, replace = TRUE, prob = probs)
...
n_psu <- function(df) dplyr::n_distinct(df$psu_id)

sample_positive_mean <- function(mu, var_mu, min_mu = 1e-8) {
  mu     <- as.numeric(mu)[1]
  var_mu <- as.numeric(var_mu)[1]
  if (!is.finite(mu) || mu <= 0) return(min_mu)
  if (!is.finite(var_mu) || var_mu <= 0) return(mu)

  sigma2_log <- log1p(var_mu / mu^2)
  meanlog    <- log(mu) - 0.5 * sigma2_log
  sdlog      <- sqrt(sigma2_log)

  sampled_mu <- stats::rlnorm(n = 1, meanlog = meanlog, sdlog = sdlog)
...
run_species_copula_sim <- function(full_df,
  ...
  simulate_one_margin <- function(df, varname, se_varname, dom, out_prefix, fixed_zero_side = c("rel", "keep")) {
    ...
    svy_design <- survey::svydesign(ids = ~psu_id, strata = ~strat_id, weights = ~wp_int, nest = TRUE, data = df)
    ...
    # Calculate bootstrap replicate uncertainty ONLY if flag == 0
    if (uncertain_flag == 0) {
      ...
      rep_theta <- safe_theta(rep_obj$reps, get_rep_var(df, varname, rep_design))
    ...
    if (verbose) {
      message("Species=", species_prefix, " | Domain=", dom, " | Case=", out_prefix, " | nrow=", nrow(df))
    }

    while (i <= n_draws) {
      if (uncertain_flag == 0) {
        sampled_mu <- sample_positive_mean(mu = mu, var_mu = var_mu)
        if (n_psu(df) <= 1) {
      ...
      if (!anyNA(sim_x)) {
        ...
        out <- data.frame(sim_id = rep(i, n_sim), my_dom_id_string = rep(dom, n_sim), stringsAsFactors = FALSE)
  ...
  simulate_joint_copula <- function(df, dom) {
    ...
    if (!is.finite(var_keep) || is.na(var_keep) || var_keep <= 0) var_keep <- mean(df[[se_keep]], na.rm = TRUE)^2
    if (!is.finite(var_rel) || is.na(var_rel) || var_rel <= 0) var_rel <- mean(df[[se_rel]], na.rm = TRUE)^2
    ...
    df_copula <- weighted_sample_for_copula(df = df, value_cols = c(keep_var, rel_var), n_sim = n_sim, weight_col = "wp_int")
    df_copula <- df_copula %>%
      dplyr::mutate(
    ...
    if (verbose) message("Species=", species_prefix, " | Domain=", dom, " | Case=keep_and_rel | nrow=", nrow(df))

    while (i <= n_draws) {
      if (uncertain_flag == 0) {
        sampled_mu_keep <- sample_positive_mean(mu = mu_keep, var_mu = var_keep)
        sampled_mu_rel  <- sample_positive_mean(mu = mu_rel, var_mu = var_rel)
        if (n_psu(df) <= 1) {
        ...
        sampled_mu_keep <- pmax(mu_keep, 1e-8)
        sampled_mu_rel  <- pmax(mu_rel, 1e-8)
        sampled_theta_keep <- theta_hat_keep_single
        sampled_theta_rel  <- theta_hat_rel_single
      }
  ...
  simulate_joint_independent <- function(df, dom) {
    ...
    if (verbose) message("Species=", species_prefix, " | Domain=", dom, " | Case=keep_and_rel_ind | nrow=", nrow(df))
  ...
  for (s in statez) {
    ...
    if (nrow(df_only_keep) > 0) {
      for (dom in unique(df_only_keep$my_dom_id_string)) {
        ...
        state_results[[paste0(species_prefix, "_only_keep_", dom)]] <- simulate_one_margin(df_dom, keep_var, se_keep, dom, "only_keep", fixed_zero_side = "rel")
      }
    }
    if (nrow(df_only_rel) > 0) {
      for (dom in unique(df_only_rel$my_dom_id_string)) {
        df_dom <- df_only_rel %>% dplyr::filter(my_dom_id_string == dom)
        state_results[[paste0(species_prefix, "_only_rel_", dom)]] <- simulate_one_margin(df_dom, rel_var, se_rel, dom, "only_rel", fixed_zero_side = "keep")
      }
    }
    if (nrow(df_joint) > 0) {
      for (dom in unique(df_joint$my_dom_id_string)) {
        df_dom <- df_joint %>% dplyr::filter(my_dom_id_string == dom)
        state_results[[paste0(species_prefix, "_keep_and_rel_", dom)]] <- simulate_joint_copula(df_dom, dom)
      }
    }
    if (nrow(df_none) > 0) {
    ...
    if (nrow(df_ind) > 0) {
      for (dom in unique(df_ind$my_dom_id_string)) {
        ...
        state_results[[paste0(species_prefix, "_keep_and_rel_ind_", dom)]] <- simulate_joint_independent(df_dom, dom)
      }
    }

    if (length(state_results) > 0) {
      species_results_all[[s]] <- dplyr::bind_rows(state_results) %>% dplyr::mutate(state = s, .before = 1)
    }
  }
  dplyr::bind_rows(species_results_all)
}

message("copula_modeling_calibration: simulating cod catch draws (", n_draws, " draws x ", n_sim, " rows per domain); this may take a while ...")
